...
class BrandModelsSettingsPage extends StatefulWidget {
  ...
  @override
  State<BrandModelsSettingsPage> createState() =>
      _BrandModelsSettingsPageState();
}
...
class _BrandModelsSettingsPageState extends State<BrandModelsSettingsPage> {
  ...
  Future<void> _initAndLoad() async {
    ...
    for (final category in _categories) {
      await _stockService.importBrandModelsFromInventory(category);
    }
    // Sonra verileri çek
                      ...
                      DropdownButtonFormField<String>(
                        ...
                        onChanged: (v) {
                          setState(() {
                            ...
                            existingBrands =
                                _brandsByCategory[tempCategory] ?? [];
                          });
                        },
                      ),
  ...
  Future<void> _showAddModelDialog(String brandName, String category) async {
    ...
    if (result == true && modelController.text.trim().isNotEmpty) {
      try {
        await _stockService.addBrandModel(
          brandName,
    ...
    final confirm = await showDialog<bool>(
      ...
      builder:
          (ctx) => AlertDialog(
            ...
            content: Text(
              '$brandName - $modelName modelini silmek istediğinize emin misiniz?',
            ),
            actions: [
              TextButton(
                onPressed: () => Navigator.pop(ctx, false),
  ...
  Future<void> _importFromInventory() async {
    try {
      final imported = await _stockService.importBrandModelsFromInventory(
        _selectedCategory,
      );
      await _loadData();
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            imported == 0
                ? 'Stoktaki urunlerden yeni model bulunamadi.'
                : '$imported model stoktan iceri aktarildi.',
          ),
          backgroundColor: imported == 0 ? Colors.orange : Colors.green,
        ),
      );
    } catch (e) {
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    // Admin kontrolü
    final canConfigure = PermissionService.hasPermission(
      _userProfile,
      AppPermission.configureStockCatalog,
    );

    if (_userProfile != null && !canConfigure) {
      return Scaffold(
        body: Center(
          child: Column(
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: OutlinedButton.icon(
                      onPressed: _importFromInventory,
                      icon: const Icon(Icons.sync_rounded),
                      label: const Text('Stoktaki Urunlerden Ice Aktar'),
