...
class _QuoteEditorPageState extends State<QuoteEditorPage> {
  ...
  /// Firma adi alanindaki RawAutocomplete icin kalici focus node.
  final _customerCompanyFocusNode = FocusNode();
  ...
  /// Firma adi alani her degistiginde cagrilir. Yazilan metin secili cariden
  /// farklilastiysa secimi birakir; kayitta _resolveCariIdForSave() o zaman
  /// yeniden eslestirir ya da yeni cari acar. Hem her zaman gorunen
  /// "Musteri Firma" alani hem de detay kartindaki "Firma Adi" alani bunu
  /// kullanir, boylece hangisinden yazilirsa yazilsin davranis ayni olur.
  void _onCompanyNameChanged(String value) {
    if (_selectedCariId.isNotEmpty) {
      final selected = _findCariById(_selectedCariId);
      if (selected == null || selected.companyName.trim() != value.trim()) {
  ...
  Future<String> _createCariFromForm() async {
    ...
    try {
      ...
      if (!mounted) return '';
      // Cari acilamazsa teklifi kaybetmiyoruz: bos id donuyoruz, teklif
      // cari_id olmadan (NULL) kaydediliyor. Hatayi tanilanabilir olmasi icin
      // tam metniyle ve uzun sureyle gosteriyoruz.
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          duration: const Duration(seconds: 12),
          content: Text(
            'Cari otomatik olusturulamadi, teklif carisiz kaydedilecek.\n$error',
          ),
        ),
      );
      return '';
  ...
  void _changeDisplayUnit(String nextUnit) {
    if (nextUnit == _selectedDisplayUnit) return;

    for (final draft in _items) {
      final unitPriceTl = _lineUnitPriceTl(draft);
      draft.unitPriceController.text = _formatUnitPriceForEditableInput(
        unitPriceTl,
        nextUnit,
      );
      draft.priceCurrencyCode = nextUnit;
    }
    setState(() => _selectedDisplayUnit = nextUnit);
  }
  ...
  Widget _buildCategoryBar() {
    ...
    return Container(
      ...
      child: Row(
        children: [
          const Icon(Icons.my_location_rounded, size: 18, color: ink),
          const SizedBox(width: 8),
          Tooltip(
            message: 'Yeni eklenen ürünler seçili kategoriye yerleştirilir.',
            child: Text(
              'Hedef kategori',
              style: Theme.of(context).textTheme.titleSmall?.copyWith(
                fontWeight: FontWeight.w800,
...
class _QuoteCodePlate extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        children: [
          const Icon(Icons.tag_rounded, size: 19, color: Color(0xFF5B6F7F)),
          const SizedBox(width: 10),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
...
class _QuoteLineEditorRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        final showDesktopRow = useDesktopLayout && constraints.maxWidth >= 1120;
